...
ALTER TABLE welcome_messages ADD COLUMN wrapper_algorithm SMALLINT;

--bun:split
-- Set the default value for the new column
ALTER TABLE welcome_messages ALTER COLUMN wrapper_algorithm SET DEFAULT 0;

--bun:split
-- Set all the existing rows to the default value
UPDATE welcome_messages SET wrapper_algorithm = 0;

--bun:split
-- Make the column non-nullable
ALTER TABLE welcome_messages ALTER COLUMN wrapper_algorithm SET NOT NULL;

--bun:split

CREATE OR REPLACE FUNCTION insert_welcome_message(installation_key BYTEA, data BYTEA, installation_key_data_hash BYTEA, hpke_public_key BYTEA, wrapper_algorithm SMALLINT)
