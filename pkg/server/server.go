...
func New(ctx context.Context, options Options) (server *Server) {
	...
	if options.Metrics.Enable {
		...
		go tracing.PanicsDo(server.ctx, "metrics server", func(_ context.Context) { server.metricsServer.Start() })
	}
	...
	if options.Metrics.Enable {
		tracing.GoPanicsDo(server.ctx, &server.wg, "status metrics", func(_ context.Context) { server.statusMetricsLoop(options) })
	}
	...
	tracing.GoPanicsDo(server.ctx, &server.wg, "static-nodes-connect-loop", func(_ context.Context) { server.staticNodesConnectLoop(options.StaticNodes) })
